 ...
 export default {
     data() {
         return {
             ...
             isTipShow: true,
             isMusic: false,
             isEnablePlayControl: 'enable', // 默认不需要播控
     ...
     methods: {
         ...
         getAudioChannel() {
             if (!this.mp || !this.mp.getCurrentAudioChannel) {
                 this.debug('no mp or ac');
                 return;
             }

             let channel = '';
             let ac = '';
             if (this.isFn(this.mp.getCurrentAudioChannel)) {
                 channel = this.mp.getCurrentAudioChannel();
             }
             this.debug('ac:' + channel);
             if (channel === 'Right') {
                 ac = '右声道';
             } else if (channel === 'Left') {
                 ac = '左声道';
             } else if (channel === 'JointStereo') {
                 ac = '混音';
         ...
         config() {
             ...
             this.debug('config in');
             if (args.length <= 0) {
                 try {
                     this.track = this.getAudioChannel();
                     this.currVolume = this.mp.getVolume();
                     this.isMute = parseInt(this.mp.getMuteFlag(), 10);
                     if (this.isFn(this.mp.getSpeed)) {
                         // this.speed = this.mp.getSpeed();
                     }
                 } catch (e) {
                     this.debug('config e:' + JSON.stringify(e));
                     console.log(e);
                 }
             } else {
                 // 有参数情况下去配置
         ...
         _init() {
             ...
             try {
                 /*
                    Maybe initialize failure, mp is null
                 */
                 this.mp = new MediaPlayer();
                 const insId = this.mp.getNativePlayerInstanceID();
                 this.mp.initMediaPlayer(
                     insId, // Player Instance ID
                     0, // PlayListFlag
                     1, // Video display mode, 1-full, 0-small
                     window.innerHeight, window.innerWidth, // video w & h (1280*720)
                     0, 0, // video position left & top
                     0, // mute flag, 0-mute 1-no mute
                     0, // use native ui flag
                     1, // subtitle flag
                     0, // video alpha
                     0, // cycle flag
                     0, // random flag
                 ...
                 console.log('mp init failed: ' + e);
                 if (typeof(e) === 'object') {
                     this.debug('catch:' + JSON.stringify(e));
                 } else {
                     this.debug('catch:' + e);
                 }
             }
         ...
         updateInfo() {
             ...
             // 更新播放状态
             if (this.isFn(this.mp.getStatus)) {
         ...
         cacheEvent(currEvent) {
             ...
             window.document.onkeypress = handler;
