...
class WebSocketConnection(asyncore.dispatcher_with_send):
    ...
    def handle_read(self):
        data = self.recv(1024)
        self.buffer += data
        if self.readState == "connecting":
            self.parse_connecting()
        elif self.readState == "open":
            self.parse_frame()
    ...
    def send_server_handshkae_10(self, headers):
        """
    ...
    def parse_frame(self):
        ...
        b2 = ord(buf[1])
        mask = b2 & 0x80 # hign bit of the second byte
        ...
        if mask:
            mask_bytes = [ord(b) for b in buf[playload_start:playload_start + 4]]
            payload_start += 4

        # is there a complete frame in the buffer ?
        if len(buf) < playload_start + length:
            return
    ...
    def send_bytes(self, bytes):
        try:
            asyncore.dispatch_with_send.send(self, bytes)
        except:
            pass
